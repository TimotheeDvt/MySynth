                ...
                while(input_file){
                        ...
                        if (std::regex_match (buffer, COMMENT_REGEX)) {
                                ...
                                switch(current_entry){
                                        case MAP_SIZE:
                                                keyboard_mapping.mapSize = std::stoi(buffer);
                                                current_entry += 1;
                                                break;
                                        case FIRST_NOTE:
                                                keyboard_mapping.firstNote = std::stoi(buffer);
                                                current_entry += 1;
                                                break;
                                        case LAST_NOTE:
                                                keyboard_mapping.lastNote = std::stoi(buffer);
                                                current_entry += 1;
                                                break;
                                        case MIDDLE_NOTE:
                                                keyboard_mapping.middleNote = std::stoi(buffer);
                                                current_entry += 1;
                                                break;
                                        case REFERENCE_NOTE:
                                                keyboard_mapping.referenceNote = std::stoi(buffer);
                                                current_entry += 1;
                                                break;
                                        case REFERENCE_FREQUENCY:
                                                keyboard_mapping.referenceFrequency = std::stod(buffer);
                                                current_entry += 1;
                                                break;
                                        case OCTAVE_DEGREE:
                                                keyboard_mapping.octaveDegree = std::stoi(buffer);
                                                current_entry += 1;
                                                break;
                                        case ACTUAL_MAP:
                                                ...
                                                if (std::regex_match (buffer, EX)) {
                                                        // A non-entry
                                                        keyboard_mapping.addMapping(KBM_NON_ENTRY);
                                                } else {
                                                        // A mapping entry
                                                        keyboard_mapping.addMapping(std::stoi(buffer));
                                                }
                                }
                        }
                }
                if (static_cast <size_t> (keyboard_mapping.mapSize) != keyboard_mapping.mapping.size()){
